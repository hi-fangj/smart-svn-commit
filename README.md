...
### 使用可执行文件版本
- Windows 10 或更高版本
- SVN 命令行工具 或 TortoiseSVN（必需，用于 SVN 操作）
...
### Windows Defender 提示"病毒"或"威胁"

这是 Windows Defender 对**未签名**可执行文件的误报，非真实病毒。

**原因**：本项目使用 PyInstaller 打包，尚未购买代码签名证书，Windows SmartScreen 会将未签名的程序标记为可疑。
...
4. 选择"添加排除项" → "文件"，选择下载的 exe 文件
```

**永久解决方案**：项目正在计划购买代码签名证书以彻底解决此问题。

### 右键菜单不显示

确认目录是 SVN 工作副本（包含 `.svn` 隐藏文件夹）。右键菜单仅 SVN 工作副本中显示。
